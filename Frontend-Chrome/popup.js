...
function initializeCategories(categoryStates) {
  ...
  categoryHeaders.forEach((header) => {
    ...
    header.addEventListener("click", () => {
      ...
      chrome.storage.local.set({ categoryStates });
    });
  });
}
...
function loadPresets() {
  chrome.storage.local.get(["filterPresets", "activePresetId"], (result) => {
...
function updateFilter(filterName, value, displayValue) {
  chrome.storage.local.set({ [filterName]: value });
  ...
  chrome.storage.local.set({ activePresetId: null });
  presetSelector.value = "";
  deletePresetBtn.disabled = true;
}
...
toneSelector.addEventListener("change", () => {
  ...
  chrome.storage.local.set({ tone: toneSelector.value, activePresetId: null });
