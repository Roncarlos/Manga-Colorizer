...
if (window.injectedMC !== 1) {
  ...
  function fetchSiteConfigurations() {
    return fetch(chrome.runtime.getURL(siteConfigFile)).then((response) =>
      response.json()
    );
  }
  ...
  function parseQuery(queryString, doc = document) {
    ...
    try {
      ...
      if (isAll) {
        ...
        const index =
          indexMatch[1] !== undefined ? parseInt(indexMatch[1], 10) : 0;
  ...
  const grayscaleMSE = (index, ctx, adjustColorBias = true) => {
    ...
    try {
      ...
      console.log(
        `[MC] [${index}] isGrayscale check error: ${insecureError}, falling back to isColoredContext`
      );
  ...
  async function fetchColorizedImg(index, url, options, img, imgName) {
    ...
    return fetch(url, options)
      ...
      .then((json) => {
        if (json.msg) console.log(`[MC] [${index}] Message: ${json.msg}`);
        if (json.colorImgData) {
          if (img.src != savedSrc) {
            console.log(
              `[MC] [${index}] Image src changed while request was in progress, invalidating...`
            );
  ...
  const setColoredOrFetch = (
    ...
    if (force || activeFetches < maxActiveFetches) {
      ...
      fetchColorizedImg(
        index,
        new URL("colorize-image-data", apiURL).toString(),
        options,
        img,
        imgName
      ).finally(() => {
  ...
  function exitSelectMode() {
    ...
    chrome.runtime.sendMessage({ action: "exitSelectMode" });
  ...
  function colorizeSingleImage(img) {
    ...
    const site = Object.keys(siteConfigurations).find((site) =>
      ...
      chrome.storage.local.get(
        ...
        (result) => {
          ...
          if (apiURL && siteConfigurations) {
            ...
            referenceImageURL =
              tone === "custom" && result.referenceImageURL
            ...
            const minImgHeight = Math.min(
              result.minImgHeight || 200,
              window.innerHeight / 2
            );
            const minImgWidth = Math.min(
              result.minImgWidth || 400,
              window.innerWidth / 2
            );

            if (storedColorTolerance > -1)
              colorTolerance = storedColorTolerance;
            ...
            const title = site ? parseQuery(config.titleQuery) : "";
            const chapter = site ? parseQuery(config.chapterQuery) : "";
            const pageNameFromAltText = site
              ? config.useAltTextAsImageName
              : false;

            console.log(
              `[MC] Website: ${site}, Title: ${title}, Chapter: ${chapter}`
            );
            ...
            images.forEach((img, index) => {
              if (img.dataset.isCloned) {
                ...
                img.addEventListener("load", colorizeMangaEventHandler, {
                  passive: true,
                });
                total--;
              } else if (
                (img.width > 0 && img.width < minImgWidth) ||
                (img.height > 0 && img.height < minImgHeight)
              ) {
                skipped++;
            ...
            console.log(
              `[MC] Report: Processing=${processing} Success=${colored} Skipped=${skipped} Failed=${failed} Awaited=${awaited} Total=${total}`
            );
          }
        }
      );
  ...
  function toggleImageVisibility(showOriginal, showColorized) {
    ...
    const clonedImages = document.querySelectorAll(
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === "updateBrightness") {
      ...
      updateBrightness(request.brightness);
    }
